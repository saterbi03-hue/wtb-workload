...
def _excluded(bundle: DataBundle) -> frozenset:
    raw = bundle.config.get("excluded_statuses")
    if raw is None:
...
def to_calendar_week(ts: pd.Timestamp) -> dict:
    iso = ts.isocalendar()
    cw_number = int(iso.week)
    cw_year = int(iso.year)
    return {
        "week": ts.strftime("%Y-%m-%d"),
        "cw": f"CW{cw_number:02d}-{cw_year}",
        "cw_label": f"CW{cw_number:02d}",
        "cw_year": cw_year,
        "cw_number": cw_number,
    }
...
## Calcule la charge hebdomadaire moyenne d’une phase. Formule ''VH = heures_totales / durée_en_semaines''
            ...
            # Stale: Détecte les phases anciennes non clôturées
            is_stale = (
                end_ts is not None
            ...
            # Effective window :Empêche le forecast de commencer dans le passé.
...
def forecast_weekly(
    ...
    # Ignore les phases terminées ou périmées.
    pi = phase_instances[
...
## Détecte quelles phases chevauchent quelles semaines.
    overlap = (
    ...
    for i in range(len(weeks)):
        ...
        if mask.any():
            ...
            for ca in np.unique(a_ca):
                if ca in row:
###### a_vh[a_ca == ca].sum() = somme des heures de charge (VH) pour un CA, capacity = capacité hebdomadaire maximale, résultat = taux de charge
                    row[ca] = float(a_vh[a_ca == ca].sum()) / capacity #### les affaires !!! + ajouter la 
        rows.append(row)
...
def forecast_monthly(weekly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ## Convertit les charges hebdomadaires en moyennes mensuelles.
...
### Compte le nombre de sujets actifs par CA.    
...
def peak_analysis(weekly_df: pd.DataFrame) -> dict[str, dict]:
    ...
    for ca in weekly_df.columns:
        series = weekly_df[ca]
## peak_val : Trouve la charge maximale atteinte.        
        peak_val = float(series.max())
## peak_ts : Trouve la semaine du maximum.            
...
###Construit une matrice : Complexité × Jalonnement
...
def smoothing_suggestions(
    ...
    # Calcule la charge moyenne de chaque CA.
...
## Arrête l’équilibrage si les charges sont proches.
...
### Mean Contribution=∑(CapacityVH × WeeksActive)/ H 
## Mesure l’impact moyen d’un sujet sur la charge.         
            ...
            temp[to_ca] = new_to
#### Mesure si le transfert améliore l’équilibre.            
...
## Quality Score=0.4×Date Coverage+0.3×Future Coverage+0.3×(1−Stale Ratio) 
## Mesure la fiabilité des données.       
        quality_score = round(
...
# Team-level decision
    ### Calcule la charge moyenne actuelle de l’équipe.
...
## Détecte une surcharge individuelle.
        if curr > 1.0:
            result.append({
                "priority": "HIGH",
                "ca_id": ca_id,
...
def data_warnings(bundle: DataBundle) -> dict:
    """Scan every CA × topic × phase for structural data problems.

    Codes:
      INVERTED_DATES – scheduled_start > scheduled_end          (ERROR)
      PARTIAL_DATES  – exactly one of start/end is null,
                       status not excluded                       (WARNING)
    """
    ...
    for _, row in bundle.topics.iterrows():
        ...
        topic_id = int(row["id"])
        topic = str(row["topic"])
        project = _opt_str(row.get("project")) or ""
        ca_disp = ca_display_map.get(ca_id, ca_id)
        ...
        for phase in PHASES:
            ...
            start_null = pd.isna(start_raw)
            end_null = pd.isna(end_raw)

            # Skip fully undated phases
            if start_null and end_null:
                continue

            status = str(status_raw).strip() if not pd.isna(status_raw) else None
            ...
            # INVERTED_DATES: both dates present but start > end
            ...
            # PARTIAL_DATES: exactly one date missing, phase not excluded
            elif start_null != end_null:
                if status is None or status not in excluded:
                    missing = "start" if start_null else "end"
                    items.append({
                        **base,
                        "severity": "WARNING",
                        "code": "PARTIAL_DATES",
                        "detail": f"{missing} date missing (status={status})",
                    })

    errors = sum(1 for w in items if w["severity"] == "ERROR")
    return {
        "warnings": items,
        "total": len(items),
        "errors": errors,
